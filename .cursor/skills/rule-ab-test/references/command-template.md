...
Create `.cursor/commands/run-rule-ab-test.md` with content like:
...
2. Create a temporary prompt file and use the skill scaffold step to create:
   `docs/technical/ab-tests/<rule-name>/<YYYY-MM-DD>-<scenario>/`
3. Save the exact prompt into `prompt.txt`.
4. Run **Result A**:
...
6. Compare A and B using the rule’s intended failure mode and success signals.
7. Create `notes.md` with:
   - Goal
   - Signals in A
   - Signals in B
   - Comparison
...
8. Update `docs/technical/rule-validation.md` with:
   - rule name
   - scenario
